...
const ratingSchema = z.object({ courseId: z.coerce.number().int().positive(), rating: z.coerce.number().int().min(1).max(5) });
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!parsed.success) throw data("Choose a rating from 1 to 5 stars.", { status: 400 });
